...
final class BASE64DecoderStreamPartial extends FilterInputStream {
    ...
    /**
     * Create a BASE64 decoder that decodes the specified input stream. The System property <code>mail.mime.base64.ignoreerrors</code> controls
     * whether errors in the encoded data cause an exception or are ignored. The default is false (errors cause exception).
     *
     * @param in the input stream
     */
    public BASE64DecoderStreamPartial(InputStream in, final boolean isPartial) {
        super(in);
        // default to false
    ...
    /**
     * NO MODIFICATION FROM ORIGINAL SOURCE. REMOVE IF CHANGED.
     *
     ...
     * NO MODIFICATION FROM ORIGINAL SOURCE. REMOVE IF CHANGED.
     *
     * static initializer for pem_convert_array.
     */
    static {
        for (int i = 0; i < 255; i++) {
     ...
     * for e0xfficiency.
     *
     * @return the next byte, -1 on EOF, or -2 if next byte is '=' (padding at end of encoded data)
     */
